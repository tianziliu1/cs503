...
int get_student(int fd, int id, student_t *s)
{
    ...
    int lseek_result = lseek(fd, myoffset, SEEK_SET);
    if(lseek_result < 0){
        return(ERR_DB_FILE);
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    result = get_student(fd, id, &mystudent);
    if(result == NO_ERROR){
    ...
    if(bytes_written < 0 || bytes_written != STUDENT_RECORD_SIZE){// TODO - What is the other condition you should check about bytes_written
...
int print_db(int fd)
{
    ...
    while(bytes_read == STUDENT_RECORD_SIZE){
        if(memcmp(&mystudent, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0){
            ...
            float gpa = mystudent.gpa/100.0;
            printf(STUDENT_PRINT_FMT_STRING, mystudent.id, mystudent.fname, mystudent.lname, gpa);
        }
    }
